  ...
  try {
    ...
    switch (event.httpMethod) {
      ...
      case "GET":
        ...
        const documentsFound = collection.find(
          { uid: contextUser.sub },
          {
      ...
      case "PATCH":
        ...
        if (body.hasOwnProperty("content"))
          updateDocument.content = body.content;
        if (body.hasOwnProperty("pin"))
          updateDocument.pin = toBoolean(body.pin);

        if (Object.keys(updateDocument).length === 1) {
          throw new Error(
            'At least specify one of "title", "content" or "pin" to update'
          );
        }
